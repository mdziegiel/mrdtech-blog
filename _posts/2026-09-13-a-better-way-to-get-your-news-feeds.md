...
IT News Sites does the real work day to day:
...
FreshRSS also exposes that Google Reader-compatible API, so any client that speaks it authenticates and pulls my subscriptions with no custom integration work, from whatever device I'm reading or watching from.
